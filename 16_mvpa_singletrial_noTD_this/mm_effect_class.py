...
decoded = ('isfamous') #label for what's decoded?
trialsets = (1,0)
resultdir = os.path.realpath(os.path.join(absolute_path,'..','mvpa_results/16_singletrial_noTD/00_mm_effect'))
print(resultdir)
...
for thisdur in durs:
    ...
    for thistrialset in trialsets:
        ...
        for mask in masks:
            ...
            if thistrialset == 1:
                accresultstr = decoded+'_mm_dur'+str(thisdur)+'_'+mask
            elif thistrialset == 0:
                accresultstr = decoded+'_nmm_dur'+str(thisdur)+'_'+mask
            ...
            for sbj in sbjs:
                ...
                if thistrialset == 1:
                    evpredresultstr = 'sub-'+sbj+'_'+decoded+'_mm_dur'+str(thisdur)+'_'+mask 
                elif thistrialset == 0:
                    evpredresultstr = 'sub-'+sbj+'_'+decoded+'_nmm_dur'+str(thisdur)+'_'+mask        
                
                sbjdir = basedir+'/sub-'+sbj
                maskdir = sbjdir+'/anat/antsreg/masks'
                datadir = sbjdir+'/func/singletrial_noTD'
                exponsetdir = basedir+'/pattern_info/singletrial'      
                
                ## read in data           
                betan,runs,tasks,picidx,isfamous,ismanmade = loadtxt(exponsetdir+'/'+sbj+'/st_pattern_info.txt',unpack=1)
                ...
                expds.sa['ismm'] = ismanmade

                expds = expds[expds.sa.ismm == thistrialset]
