...
#SBATCH --mem=5000 --output=logs/katzer3_09_run_singletrial_noTD_level1_%j.out
...
do
	for r in ${rnr}
	do
		sbjdir=~/Data/katzer3/sub-${sbj}/func/singletrial_noTD
		mkdir -p ${sbjdir}
	
		outfsf=${sbjdir}/${r}_${d}_noTD_level1.fsf
		sed -e "s|SUBNUM|${sbj}|g" -e "s|RUN|${r}|g" -e "s|DUR|${d}|g" <~/Data/katzer3/scripts/feat_templates/singletrial_noTD.fsf>${outfsf}
		feat ${outfsf}
	done
done
